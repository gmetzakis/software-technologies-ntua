...
export async function SessionsPerProvider(args){

    let myArray = process.argv.slice(3)

    if (typeof args.provider === 'boolean' || typeof args.provider === 'undefined' || typeof args.provider != 'number'){
        console.log('You have to set parameter --provider using a number')
        return
    }
    ...
    if ((myArray.length > 8 && args.format) || (myArray.length > 6 && (!args.format))){
        ...
        console.log('SessionsPerProvider --provider ID --datefrom Date --dateto Date --format Format')
        console.log('*You can skip parameter --format. By default json format is selected.')
        return
    }

    var url = 'http://localhost:8765/evcharge/api/SessionsPerProvider'
    url = url + '/' + args.provider + '/' + args.datefrom + '/' + args.dateto
