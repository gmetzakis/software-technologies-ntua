...
export async function SessionsPerPoint(args){

    let myArray = process.argv.slice(3)

    if (typeof args.point === 'boolean' || typeof args.point === 'undefined' || typeof args.point != 'number'){
        console.log('You have to set parameter --point using a number')
        return
    }
    ...
    if ((myArray.length > 8 && args.format) || (myArray.length > 6 && (!args.format))){
        ...
        console.log('SessionsPerPoint --point ID --datefrom Date --dateto Date --format Format')
        console.log('*You can skip parameter --format. By default json format is selected.')
        return
    }

    var url = 'http://localhost:8765/evcharge/api/SessionsPerPoint'
    url = url + '/' + args.point + '/' + args.datefrom + '/' + args.dateto
