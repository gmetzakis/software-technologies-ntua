let axios,type;_e08‍.x([["SessionsPerStation",()=>SessionsPerStation]]);_e08‍.w("axios",[["default",["axios"],function(v){axios=v}]]);_e08‍.w("os",[["type",["type"],function(v){type=v}]]);

var fs = require('fs'); 

       async function SessionsPerStation(args){
    ...
    function request(url, path){
        ...
        function get_request(err, data){
            ...
            .then(response => {
                _e08‍.g.console.log(response.data[0])
            })
            .catch((error) => {
                _e08‍.g.console.log("Error", error.response.status)
                _e08‍.g.console.log(error.response.data.message)
            })
        })
    }
