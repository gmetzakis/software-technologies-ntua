...
var get_SessionsPerPoint = require('../../model/getSessions/getSessionsPerPointModel.js');
var authenticate = require('../../auth/auth.js');

const checks = require("../utils.js");

exports.get_sessions_per_point = function(req, res){
  var year_from;
  var month_from;
  var day_from;
  var year_to;
  ...
  if(!(req.params.pointID && req.params.date_from && req.params.date_to)){
    res.status(400).json({"success":false,"message":"One or more mandatory fileds are missing!"});
  }
  else if(req.params.pointID.length>255 || req.params.length > 255){
    ...
    authenticate(req, function(err2, res2, usr2, per){
      if(err2){
        ...
        get_SessionsPerPoint(req.params.pointID,year_from,month_from,day_from,year_to,month_to,day_to,dateTime,function(err3, res3){
          if(err3){
